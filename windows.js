  ...
  <iframe id="browserFrame" src="https://www.google.com/webhp?igu=1" style="flex:1;border:none;"></iframe>
   ...
   sterlingia: {
    ...
    "imageviewer": {
        ...
        icon: "https://www.thiings.co/_next/image?url=https%3A%2F%2Flftz25oez4aqbxpq.public.blob.vercel-storage.com%2Fimage-DPM2VMQ9vdZ4HcXLRwCFZ7FK5Fn7Bx.png&w=1000&q=75",
        width: 900, height: 600, type: "imageviewer"
    }
};
    ...
    root: {
        ...
        contents: {
            ...
            Imágenes: {
                type: "folder",
                contents: {
                    "Tahoe Light.jpg":  { type: "image", src: "https://512pixels.net/wp-content/uploads/2025/06/26-Tahoe-Light-6K-thumb.jpeg",  modified: "Hoy" },
                    "Tahoe Dark.jpg":   { type: "image", src: "https://512pixels.net/wp-content/uploads/2025/06/26-Tahoe-Dark-6K-thumb.jpeg",   modified: "Hoy" },
                    "Beach Dawn.jpg":   { type: "image", src: "https://512pixels.net/wp-content/uploads/2025/08/26-Tahoe-Beach-Dawn-thumb.jpeg",  modified: "Hoy" },
                    "Beach Day.jpg":    { type: "image", src: "https://512pixels.net/wp-content/uploads/2025/08/26-Tahoe-Beach-Day-thumb.jpeg",   modified: "Hoy" },
                    "Beach Dusk.jpg":   { type: "image", src: "https://512pixels.net/wp-content/uploads/2025/08/26-Tahoe-Beach-Dusk-thumb.jpeg",  modified: "Hoy" },
                    "Beach Night.jpg":  { type: "image", src: "https://512pixels.net/wp-content/uploads/2025/08/26-Tahoe-Beach-Night-thumb.jpeg", modified: "Hoy" },
                    "Avatar.jpg":       { type: "image", src: "https://avatars.githubusercontent.com/u/98182135?v=4",                            modified: "Hoy" }
                }
            }
        }
    }
...
function openApp(appName, extra = null) {
    ...
    if (appName === "explorer") {
        ...
        loadRecycleBin(win);
    } else if (appName === "imageviewer" || app.type === "imageviewer") {
        loadImageViewer(win, extra);
...
/* ============================================================
   DOCK AUTO-HIDE SYSTEM
============================================================ */
const dockEl       = document.getElementById("dock");
const dockGhost    = document.getElementById("dockGhostBtn");
const dockHoverZone = document.getElementById("dockHoverZone");
let dockHideTimeout  = null;
let dockPeekTimeout  = null;
let dockShouldHide   = false;

function hasVisibleWindows() {
    return Object.values(activeWindows).some(({ win }) => win.style.display !== "none");
}

function checkDockVisibility() {
    const shouldHide = hasVisibleWindows();
    if (shouldHide === dockShouldHide) return;
    dockShouldHide = shouldHide;

    if (shouldHide) {
        // Small delay before hiding so it feels natural
        dockHideTimeout = setTimeout(() => {
            dockEl.classList.add("dock-hidden");
            dockEl.classList.remove("dock-peek");
            dockGhost.classList.add("visible");
            dockHoverZone.classList.add("active");
        }, 600);
    } else {
        clearTimeout(dockHideTimeout);
        dockEl.classList.remove("dock-hidden", "dock-peek");
        dockGhost.classList.remove("visible");
        dockHoverZone.classList.remove("active");
    }
}

function peekDock() {
    if (!dockShouldHide) return;
    clearTimeout(dockPeekTimeout);
...
function unpeekDock() {
    if (!dockShouldHide) return;
    dockPeekTimeout = setTimeout(() => {
        if (!dockEl.matches(":hover") && !dockGhost.matches(":hover") && !dockHoverZone.matches(":hover")) {
            dockEl.classList.remove("dock-peek");
            dockEl.classList.add("dock-hidden");
        }
    }, 600);
...
dockGhost.addEventListener("click",      peekDock);
dockGhost.addEventListener("mouseenter", peekDock);
dockGhost.addEventListener("mouseleave", unpeekDock);

// Hover zone at very bottom of screen
dockHoverZone.addEventListener("mouseenter", peekDock);
dockHoverZone.addEventListener("mouseleave", unpeekDock);

// Keep dock visible while mouse is over it
dockEl.addEventListener("mouseenter", () => { clearTimeout(dockPeekTimeout); });
dockEl.addEventListener("mouseleave", unpeekDock);
...
document.addEventListener("click", (e) => {
    ...
    if (!e.target.closest(".mb-dropdown-trigger, .mb-dropdown")) {
        closeMenuDropdowns?.();
    }
});
...
function openMenuDropdown(trigger, menuId) {
    const menu = document.getElementById(menuId);
    if (!menu) return;
    if (activeMenuDropdown === menu) { closeMenuDropdowns(); return; }
    closeMenuDropdowns();
    trigger.classList.add("mb-active");
    menu.classList.add("mb-open");
    activeMenuDropdown = menu;
    // Refresh open windows list
    if (menuId === "mbMenuVentana") refreshWindowsMenu();
}

function closeMenuDropdowns() {
    document.querySelectorAll(".mb-dropdown.mb-open").forEach(m => m.classList.remove("mb-open"));
    document.querySelectorAll(".menubar-menu-item.mb-active").forEach(t => t.classList.remove("mb-active"));
    activeMenuDropdown = null;
}

// Bind triggers
document.querySelectorAll(".mb-dropdown-trigger").forEach(trigger => {
    trigger.addEventListener("click", (e) => {
        e.stopPropagation();
        closeAllPanels();
        openMenuDropdown(trigger, trigger.dataset.menu);
    });
    // Hover to switch between open menus (only if one is already open)
    trigger.addEventListener("mouseenter", () => {
        if (activeMenuDropdown) openMenuDropdown(trigger, trigger.dataset.menu);
    });
});

document.addEventListener("click", (e) => {
    if (!e.target.closest(".mb-dropdown-trigger") && !e.target.closest(".mb-dropdown")) {
        closeMenuDropdowns();
    }
});

function refreshWindowsMenu() {
    const container = document.getElementById("mbOpenWindows");
    if (!container) return;
    const open = Object.entries(activeWindows).filter(([, { win }]) => win.style.display !== "none");
    if (open.length === 0) {
        container.innerHTML = `<li class="mb-dimmed">Sin ventanas abiertas</li>`;
        return;
    }
    container.innerHTML = open.map(([k, { win }]) => {
        const app = apps[k];
        return `<li data-action="focusWin" data-win="${k}"><img src="${app?.icon || ''}" style="width:14px;height:14px;object-fit:contain;border-radius:3px;"> ${app?.title || k}</li>`;
    }).join("");
    container.querySelectorAll("[data-action='focusWin']").forEach(li => {
        li.onclick = () => { closeMenuDropdowns(); const w = activeWindows[li.dataset.win]?.win; if (w) { restoreWindow(w); focusWindow(w); } };
    });
}

// Handle dropdown actions
document.querySelectorAll(".mb-dropdown").forEach(menu => {
    menu.addEventListener("click", (e) => {
        ...
        const actions = {
            newFolder: () => {
                if (!activeWindows["explorer"]) openApp("explorer");
                setTimeout(() => {
                    const w = activeWindows["explorer"]?.win;
                    if (w) w.querySelector("#fnNewFolder")?.click();
                }, 300);
            },
            newFile: () => {
                if (!activeWindows["explorer"]) openApp("explorer");
                setTimeout(() => {
                    const w = activeWindows["explorer"]?.win;
                    if (w) w.querySelector("#fnNewFile")?.click();
                }, 300);
            },
            newWindow: () => {
                const id = "explorer_" + Date.now();
                apps[id] = { ...apps.explorer, title: "Finder" };
                openApp(id);
            },
            openFinder:      () => openApp("explorer"),
            openBrowser:     () => openApp("browser"),
            openNotes:       () => openApp("notes"),
            openImageViewer: () => openApp("imageviewer"),
            closeWindow:     () => { if (focused) closeWindow(focused); },
            closeAll:        () => { Object.values(activeWindows).forEach(({ win }) => closeWindow(win)); },
            wallpaper:       () => openPanel("wallpaperPanel"),
            ...
            search:  () => {
                if (focused) focused.querySelector("#fnSearch")?.focus();
                else { openPanel("spotlight"); document.getElementById("spotlightInput")?.focus(); }
            },
            viewIcons: () => focused?.querySelector("#fnViewGrid")?.click(),
            viewList:  () => focused?.querySelector("#fnViewList")?.click(),
            showHideSidebar: () => focused?.querySelector("#ivToggleThumbs, #fnNewFolder")?.closest(".finder-container")?.querySelector(".finder-sidebar")?.classList.toggle("iv-hidden"),
            missionControl: () => showMissionControl(),
            sortName: () => showToast("Finder", "Ordenado por nombre"),
            sortDate: () => showToast("Finder", "Ordenado por fecha"),
            sortKind: () => showToast("Finder", "Ordenado por tipo"),
            goBack:   () => { if (focused) { focused.querySelector("#fnBack")?.click(); } },
            goForward: () => { if (focused) { focused.querySelector("#fnFwd")?.click(); } },
            goHome:   () => {
                if (!activeWindows["explorer"]) openApp("explorer");
                else { const w = activeWindows["explorer"]?.win; w?.querySelector("[data-nav='home']")?.click(); focusWindow(w); }
            },
            goDocs:   () => {
                if (!activeWindows["explorer"]) openApp("explorer");
                setTimeout(() => activeWindows["explorer"]?.win?.querySelector("[data-nav='docs']")?.click(), 200);
            },
            goImgs:   () => {
                if (!activeWindows["explorer"]) openApp("explorer");
                setTimeout(() => activeWindows["explorer"]?.win?.querySelector("[data-nav='img']")?.click(), 200);
            },
            goMusic:  () => {
                if (!activeWindows["explorer"]) openApp("explorer");
                setTimeout(() => activeWindows["explorer"]?.win?.querySelector("[data-nav='music']")?.click(), 200);
            },
            goVideos: () => {
                if (!activeWindows["explorer"]) openApp("explorer");
                setTimeout(() => activeWindows["explorer"]?.win?.querySelector("[data-nav='video']")?.click(), 200);
            },
            goPortfolio: () => window.location.href = "index.html",
            goProjects:  () => window.location.href = "proyectoApp.html",
            minimizeWin:  () => { if (focused) minimizeWindow(focused); },
            maximizeWin:  () => { if (focused) toggleMaximize(focused); },
...
document.addEventListener("keydown", (e) => {
    ...
    if (mod && e.key === "n") { e.preventDefault(); openApp("explorer"); }
    if (mod && e.key === "w") { e.preventDefault(); const f = getFocusedWindow(); if (f) closeWindow(f); }
    if (mod && e.key === "m") { e.preventDefault(); const f = getFocusedWindow(); if (f) minimizeWindow(f); }
    if (mod && e.key === "1") { e.preventDefault(); getFocusedWindow()?.querySelector("#fnViewGrid")?.click(); }
    if (mod && e.key === "2") { e.preventDefault(); getFocusedWindow()?.querySelector("#fnViewList")?.click(); }
});
        ...
        if (filtered.length === 0) {
            ...
            filtered.forEach(([itemName, item]) => {
                ...
                if (viewMode === "list") {
                    ...
                    el.innerHTML = `
                      <img src="${item.type === "image" ? item.src : icon}" alt="${itemName}" style="${item.type === "image" ? "border-radius:3px;object-fit:cover;" : ""}">
                      <span class="finder-list-col-name">${itemName}</span>
                      <span class="finder-list-col-date finder-list-meta">${item.modified || "Hoy"}</span>
                      <span class="finder-list-col-kind finder-list-meta">${item.type === "folder" ? "Carpeta" : item.type === "image" ? "Imagen" : "Texto"}</span>
                      <span class="finder-list-col-size finder-list-meta">${getFileSize(item)}</span>
                    `;
                } else {
                    el.className = "finder-item";
                    const displayIcon = item.type === "image" ? item.src : icon;
                    const imgStyle = item.type === "image" ? "border-radius:8px;object-fit:cover;width:56px;height:56px;" : "";
                    el.innerHTML = `
                      <img src="${displayIcon}" alt="${itemName}" style="${imgStyle}">
                      <span>${itemName}</span>
...
/* ============================================================
   IMAGE VIEWER
============================================================ */
function loadImageViewer(win, startImageName = null) {
    const area = win.querySelector(".app-area");
    area.style.padding = "0";

    const imgFolder = fileSystem.root.contents["Imágenes"];

    function getImages() {
        return Object.entries(imgFolder.contents || {})
            .filter(([, item]) => item.type === "image")
            .map(([name, item]) => ({ name, src: item.src }));
    }

    let images = getImages();
    let currentIdx = startImageName ? images.findIndex(i => i.name === startImageName) : 0;
    ...
    area.innerHTML = `
      <div class="iv-container">
        <!-- Sidebar thumbnails -->
        <div class="iv-sidebar" id="ivSidebar">
          <div class="iv-sidebar-header">
            <span class="iv-sidebar-title"><i class="fi fi-sr-images"></i> Imágenes</span>
            <button class="iv-icon-btn" id="ivAddImg" title="Agregar imagen"><i class="fi fi-sr-add-image"></i></button>
          </div>
          <div class="iv-thumb-list" id="ivThumbList"></div>
        </div>
        <!-- Main viewer -->
        <div class="iv-main">
          <div class="iv-toolbar">
            <div class="iv-toolbar-left">
              <button class="iv-icon-btn" id="ivPrev" title="Anterior (←)"><i class="fi fi-sr-angle-left"></i></button>
              <span class="iv-counter" id="ivCounter">1 / 1</span>
              <button class="iv-icon-btn" id="ivNext" title="Siguiente (→)"><i class="fi fi-sr-angle-right"></i></button>
            </div>
            <div class="iv-filename" id="ivFilename">-</div>
            <div class="iv-toolbar-right">
              <button class="iv-icon-btn" id="ivZoomOut" title="Reducir (-)"><i class="fi fi-sr-search-minus"></i></button>
              <span class="iv-zoom-label" id="ivZoomLabel">100%</span>
              <button class="iv-icon-btn" id="ivZoomIn" title="Ampliar (+)"><i class="fi fi-sr-search-plus"></i></button>
              <button class="iv-icon-btn" id="ivFit" title="Ajustar (0)"><i class="fi fi-sr-expand"></i></button>
              <div class="iv-sep"></div>
              <button class="iv-icon-btn" id="ivRotateL" title="Rotar izquierda"><i class="fi fi-sr-rotate-left"></i></button>
              <button class="iv-icon-btn" id="ivRotateR" title="Rotar derecha"><i class="fi fi-sr-rotate-right"></i></button>
              <button class="iv-icon-btn" id="ivFlipH" title="Voltear horizontal"><i class="fi fi-sr-horizontal-rule"></i></button>
              <button class="iv-icon-btn" id="ivFlipV" title="Voltear vertical"><i class="fi fi-sr-vertical-rule"></i></button>
              <div class="iv-sep"></div>
              <button class="iv-icon-btn" id="ivSlideshow" title="Presentación"><i class="fi fi-sr-play"></i></button>
              <button class="iv-icon-btn" id="ivFullscreen" title="Pantalla completa"><i class="fi fi-sr-arrows-maximize"></i></button>
              <button class="iv-icon-btn" id="ivToggleThumbs" title="Ocultar/Mostrar miniaturas"><i class="fi fi-sr-sidebar"></i></button>
              <button class="iv-icon-btn iv-delete" id="ivDelete" title="Eliminar imagen"><i class="fi fi-sr-trash-xmark"></i></button>
            </div>
          </div>
          <div class="iv-stage" id="ivStage">
            <div class="iv-empty" id="ivEmpty" style="display:none;">
              <i class="fi fi-sr-picture" style="font-size:64px;opacity:0.15;"></i>
              <p>No hay imágenes en la carpeta</p>
              <button class="finder-action-btn accent" id="ivEmptyAdd" style="margin-top:12px;">Agregar imagen</button>
            </div>
            <img class="iv-image" id="ivImage" src="" alt="" draggable="false">
            <!-- Zoom & nav overlay arrows -->
            <div class="iv-nav-arrow iv-left" id="ivArrowLeft">‹</div>
            <div class="iv-nav-arrow iv-right" id="ivArrowRight">›</div>
            <!-- Slideshow badge -->
            <div class="iv-slideshow-badge" id="ivSlideBadge" style="display:none;"><i class="fi fi-sr-play"></i> Presentación</div>
          </div>
          <!-- Filmstrip at bottom -->
          <div class="iv-filmstrip" id="ivFilmstrip"></div>
        </div>
      </div>
      <!-- Hidden file input for adding images -->
      <input type="file" id="ivFileInput" accept="image/*" multiple style="display:none;">
    `;

    const img = area.querySelector("#ivImage");
    const stage = area.querySelector("#ivStage");
    const counter = area.querySelector("#ivCounter");
    const filenameEl = area.querySelector("#ivFilename");
    const zoomLabel = area.querySelector("#ivZoomLabel");
    const thumbList = area.querySelector("#ivThumbList");
    const filmstrip = area.querySelector("#ivFilmstrip");
    const emptyEl = area.querySelector("#ivEmpty");
    const sidebar = area.querySelector("#ivSidebar");

    function applyTransform() {
        const scaleX = flipH ? -zoom : zoom;
        const scaleY = flipV ? -zoom : zoom;
        img.style.transform = `translate(${panX}px, ${panY}px) rotate(${rotation}deg) scale(${scaleX}, ${scaleY})`;
    ...
    function showImage(idx) {
        ...
        if (images.length === 0) {
            ...
            area.querySelector("#ivArrowLeft").style.display = "none";
            area.querySelector("#ivArrowRight").style.display = "none";
            counter.textContent = "0 / 0";
            filenameEl.textContent = "";
            renderThumbs(); renderFilmstrip();
            return;
        }
        ...
        area.querySelector("#ivArrowLeft").style.display = images.length > 1 ? "flex" : "none";
        area.querySelector("#ivArrowRight").style.display = images.length > 1 ? "flex" : "none";

        const cur = images[currentIdx];
        img.src = cur.src;
        img.alt = cur.name;
        counter.textContent = `${currentIdx + 1} / ${images.length}`;
        filenameEl.textContent = cur.name;
        resetTransform();
        renderThumbs();
        renderFilmstrip();
    }
    ...
    function renderThumbs() {
        ...
        images.forEach((im, i) => {
            ...
            t.className = "iv-thumb" + (i === currentIdx ? " active" : "");
            t.title = im.name;
            t.innerHTML = `<img src="${im.src}" alt="${im.name}"><span>${im.name}</span>`;
    ...
    function renderFilmstrip() {
        ...
        images.forEach((im, i) => {
            const t = document.createElement("div");
            t.className = "iv-film-item" + (i === currentIdx ? " active" : "");
            t.title = im.name;
            t.innerHTML = `<img src="${im.src}" alt="${im.name}">`;
            t.onclick = () => showImage(i);
            filmstrip.appendChild(t);
        });
        // Scroll active into view
        setTimeout(() => filmstrip.querySelector(".active")?.scrollIntoView({ behavior: "smooth", inline: "center", block: "nearest" }), 50);
    }

    // Zoom
    area.querySelector("#ivZoomIn").onclick = () => { zoom = Math.min(zoom + 0.2, 5); applyTransform(); };
    area.querySelector("#ivZoomOut").onclick = () => { zoom = Math.max(zoom - 0.2, 0.1); applyTransform(); };
    area.querySelector("#ivFit").onclick = resetTransform;
    ...
    document.addEventListener("mouseup", () => { isPanning = false; stage.style.cursor = ""; });

    // Rotate
    area.querySelector("#ivRotateL").onclick = () => { rotation -= 90; applyTransform(); };
    area.querySelector("#ivRotateR").onclick = () => { rotation += 90; applyTransform(); };

    // Flip
    area.querySelector("#ivFlipH").onclick = () => { flipH = !flipH; applyTransform(); };
    area.querySelector("#ivFlipV").onclick = () => { flipV = !flipV; applyTransform(); };

    // Navigation
    area.querySelector("#ivPrev").onclick = () => showImage(currentIdx - 1);
    area.querySelector("#ivNext").onclick = () => showImage(currentIdx + 1);
    area.querySelector("#ivArrowLeft").onclick = () => showImage(currentIdx - 1);
    area.querySelector("#ivArrowRight").onclick = () => showImage(currentIdx + 1);

    // Delete
    area.querySelector("#ivDelete").onclick = () => {
        if (images.length === 0) return;
        const name = images[currentIdx].name;
        delete imgFolder.contents[name];
        images = getImages();
        showImage(Math.min(currentIdx, images.length - 1));
        showToast("Visor de imágenes", `"${name}" eliminada`);
    };

    // Slideshow
    const slideBadge = area.querySelector("#ivSlideBadge");
    const slideshowBtn = area.querySelector("#ivSlideshow");
    function stopSlideshow() {
        slideshow = false;
        clearInterval(slideshowInterval);
        slideshowBtn.innerHTML = '<i class="fi fi-sr-play"></i>';
    ...
    slideshowBtn.onclick = () => {
        if (slideshow) {
            ...
            slideshow = true;
            slideshowBtn.innerHTML = '<i class="fi fi-sr-stop"></i>';
            slideBadge.style.display = "flex";
            slideshowInterval = setInterval(() => showImage(currentIdx + 1), 3000);
        }
    };

    // Fullscreen
    area.querySelector("#ivFullscreen").onclick = () => {
        const el = win;
    ...
    // Toggle sidebar
    area.querySelector("#ivToggleThumbs").onclick = () => {
        thumbsVisible = !thumbsVisible;
        sidebar.style.display = thumbsVisible ? "" : "none";
    };

    // Add images via file input
    const fileInput = area.querySelector("#ivFileInput");
    const addBtn = area.querySelector("#ivAddImg");
    const emptyAdd = area.querySelector("#ivEmptyAdd");
    const doAdd = () => fileInput.click();
    addBtn.onclick = doAdd;
    if (emptyAdd) emptyAdd.onclick = doAdd;
    fileInput.onchange = (e) => {
        ...
        files.forEach(file => {
            ...
            reader.onload = (ev) => {
                ...
                let uniqueName = name;
                let c = 2;
                while (imgFolder.contents[uniqueName]) uniqueName = name.replace(/(\.[^.]+)?$/, ` (${c++})$1`);
                imgFolder.contents[uniqueName] = { type: "image", src: ev.target.result, modified: "Ahora" };
                images = getImages();
                showImage(images.length - 1);
                showToast("Visor de imágenes", `"${uniqueName}" agregada`);
            };
            reader.readAsDataURL(file);
        });
        fileInput.value = "";
    ...
    win.addEventListener("keydown", (e) => {
        ...
        else if (e.key === "ArrowRight") showImage(currentIdx + 1);
        else if (e.key === "+" || e.key === "=") { zoom = Math.min(zoom + 0.2, 5); applyTransform(); }
        else if (e.key === "-") { zoom = Math.max(zoom - 0.2, 0.1); applyTransform(); }
